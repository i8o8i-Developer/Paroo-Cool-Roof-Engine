...
```mermaid
graph TD
    A[Copernicus Sentinel-2 Optical BOA] --> E[PARoo Master Engine]
    B[Google Open Buildings v3 2.5D] --> E
    C[USGS Landsat 8/9 TIRS Day LST] --> E
    D[NASA JPL ECOSTRESS Diurnal Radiometry] --> E
    F[Official Census 2011 Table H-02] --> E
    G[WorldPop 100m Demographic Grid] --> E
    
    E --> H[Stage 1: Multi-Source Spatial Ingestion & Alignment]
    H --> I[Stage 2: Multispectral Indices & GLCM Texture Extractor]
    I --> J[Stage 3: Weakly Supervised LLP Dirichlet Classifier]
    J --> K[Stage 4: Thermal Energy Balance & Radiative Physics Cross-Check]
    K --> L[Stage 5: Multi-Factor Composite Heat-Risk Scoring Index]
    L --> M[Stage 6: Bounded Knapsack Pareto Budget Optimizer]
    
    M --> N[Relational SQLite Persistent Database]
    M --> O[Google Gemini 1.5 Flash AI Briefing Synthesis]
    M --> P[Interactive Leaflet Cyber-GIS Web Studio]
    M --> Q[Contractor-Ready Line-Item CSV Work Order Export]
```
...
```mermaid
graph TD
    A[User Selects 'Draw AOI' Tool] --> B[Leaflet.Draw Rectangle Event]
    B --> C[Extract WGS84 Bounding Box: MinLon, MinLat, MaxLon, MaxLat]
    C --> D[Trigger Neon Multi-Stage HUD Progress Bar: 0% -> 100%]
    D --> E[POST /Api/Footprints/CustomAOI]
    
    E --> F[Backend Ingests 850 - 1,024 Dense Polygons]
    F --> G[Extracts Sentinel-2, Landsat, ECOSTRESS, Census & WorldPop]
    G --> H[Solves Knapsack Allocation Under Current Budget Cap]
    
    H --> I[Returns GeoJSON FeatureCollection + Analytics]
    I --> J[Leaflet Renders 1,024 Vector Polygons]
    I --> K[FlyTo Camera Animation Fits Bounding Box]
    I --> L[Update Work-Order Table & Executive Briefing]
    I --> M[Header Retains Custom AOI Name & Stats]
```
